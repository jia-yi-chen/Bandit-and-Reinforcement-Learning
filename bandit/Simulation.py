	...
	def runAlgorithms(self, algorithms):
		...
		for iter_ in range(self.testing_iterations):
			...
			for u in self.users:
				...
				# get optimal reward for user u at time t
				...
				for alg_name, alg in algorithms.items():
					...
					# apply At to get real reward Rt ~ N( theta0_u_k=q*k , 1 )
					reward = self.getReward(u, pickedArticle) + noise

					#
					alg.updateParameters(pickedArticle, reward, u.id)

					# if alg_name=='ThompsonSampling':
					# 	print("Rt=", reward, "At=", pickedArticle.id)
					# 	print("Real theta =", {article.id:np.dot(u.theta, article.featureVector) for article in self.articlePool})
					# 	print("Posterior Mean", alg.users[u.id].posterior_mean)
					# 	print("Posterior Sigma", alg.users[u.id].posterior_sigma)
					# elif alg_name=='EpsilonGreedyMultiArmedBandit':
					# 	pass
					# print('\n')
...
if __name__ == '__main__':
	...
	algorithms['PHE'] = PHE(num_arm=n_articles, noiseScale=NoiseScale)
